...
//
// Build with: CGO_ENABLED=0 go build ./cmd
package np
...
func NewCmd() *cobra.Command {
	...
	// container engine (docker or podman) - now only used for fallback cleanup
	cmd.Flags().StringVar(&flags.ContainerEngine, "container-engine", auditpkg.GetContainerToolFromEnvVar(),
		fmt.Sprintf("Container tool to use for cleanup (options: %s, %s)", auditpkg.Docker, auditpkg.Podman))
...
func getAllBundleImages(indexImage string) ([]string, error) {
	...
	// This avoids image registry dependencies while giving us structured data
...
func processBundleWorker(jobs <-chan bundleJob, results chan<- bundleResult) {
	for job := range jobs {
		...
		if _, err := os.Stat(bundleDir); err == nil {
			...
			defer os.RemoveAll(workerCacheDir)
		}

		// The bundle manifests are now directly in bundleDir (no need for bundle/ subdirectory)
		...
		filepath.Walk(bundleRoot, func(filePath string, info os.FileInfo, err error) error {
			...
			n, _ := f.Read(buf)
			data := buf[:n]
			// skip binary files (now with extension check for faster detection)
...
func isBinary(filePath string, data []byte) bool {
	// Skip known binary file extensions immediately (Jordan's approach)
	...
	// Only check first 512 bytes for null bytes (faster)
	sampleSize := 512
